...
Purpose:
--------
Transform GeoJSON files into arrays by checking if coordinates in lat/lon arrays
fall within polygons. Outputs both brightness and lat/lon cut arrays.
"""
...
if __name__ == "__main__":
    ...
    channel_arr = np.array([np.array([
        ...
        ("uncut_all = np.stack((lon_arr, lat_arr, bright_arr))[:, slicey, slicex]; "
         "global all_arr; "
         "all_arr = np.full(shape=(3, 1024, 1024), "
         "fill_value=np.array([np.nan, np.nan, 10])[:, np.newaxis, np.newaxis]); "
         ...
         "np.savez_compressed(f'{save_path}_cut', brightness=all_arr[2]); "
         "np.savez_compressed(f'{path_dict.get(online)}cataloging/nc_process/geojson_transform/completed_"
         "arrays/latlon_arrs/' "
         "+ file[path_len: path_len + 22] + '_latlon', lat=all_arr[1], lon=all_arr[0])")
    ])])
    ...
    for channel in channel_arr:
        ...
        done_files = set(glob.glob(f"{path_dict[online]}cataloging/nc_process/geojson_transform/completed_"
                                   f"arrays/{channel[0]}/*.npz"))
        # Normalize paths for comparison
        done_files = {f.replace('geojson_transform/completed_arrays',
                                'plotting').replace('_cut', '') for f in done_files}
